...
type Props = {
  ...
  initial?: Movie | null;
  onClose: () => void;
  onSwitch?: (m: Movie) => void;
};

export function MovieModal({ movieId, initial, onClose, onSwitch }: Props) {
  const items = useWatchlist();
  const item = movieId ? items.find((i) => i.id === movieId) : null;
  const inList = !!item;
  const [showTrailer, setShowTrailer] = useState(false);

  const apiKey = useTmdbKey();
  const detailsFn = useServerFn(tmdbDetails);
  const { data, isLoading } = useQuery({
    queryKey: ["movie", movieId, !!apiKey],
    queryFn: () => detailsFn({ data: { id: movieId!, apiKey } }),
    enabled: !!movieId,
    staleTime: 5 * 60_000,
  });

  const movie: (Movie & { similar?: Movie[] }) | null =
    data ?? (initial ? { ...initial, similar: [] } : null);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-2xl glass neon-border animate-scale-in scrollbar-hide">
          ...
          {showTrailer && movie.trailerId ? (
            ...
            <>
              ...
              {movie.trailerId && (
                ...
                >
                  <span
                    className="size-20 rounded-full glass neon-border flex items-center justify-center group-hover:glow-cyan transition"
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 size-9 rounded-full glass flex items-center justify-center hover:glow-cyan transition"
        >
          <X className="size-4" />
        </button>
        ...
        <div className="p-6 sm:p-8 grid sm:grid-cols-[200px_1fr] gap-6">
          <img
            src={movie.poster}
            alt={movie.title}
            className="rounded-xl w-full sm:w-[200px] aspect-[2/3] object-cover glow-purple -mt-24 sm:-mt-32 relative z-10"
          />
          <div>
            <h2 className="font-display text-3xl sm:text-4xl font-bold">{movie.title}</h2>
            {movie.tagline && <p className="text-cyan italic mt-1">{movie.tagline}</p>}
            ...
            <p className="mt-5 text-sm leading-relaxed text-foreground/90">{movie.overview || (isLoading ? "Loading transmission…" : "No overview available.")}</p>
            ...
            <div className="mt-6 flex flex-wrap gap-3 items-center">
              ...
              <button
                ...
                className={`size-11 rounded-lg glass neon-border flex items-center justify-center transition ${item?.favorite ? "text-favorite" : "hover:text-favorite"}`}
                aria-label="Favorite"
              >
