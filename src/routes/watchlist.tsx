...
function WatchlistPage() {
  ...
  const onDrop = (targetId: string) => {
    ...
    const ids = list.map((x) => x.movie.id);
    const from = ids.indexOf(sourceId);
    const to = ids.indexOf(targetId);
    if (from < 0 || to < 0) return;
    ids.splice(to, 0, ids.splice(from, 1)[0]);
    // merge back with full watchlist order (only for "all" tab)
      ...
      {list.length === 0 ? (
        <div className="glass neon-border rounded-2xl p-12 sm:p-20 text-center relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <h2 className="font-display text-2xl sm:text-3xl font-bold">{tab === "all" ? "Your archive is empty" : `No ${tab} yet`}</h2>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {it.favorite && <Heart className="size-4 fill-current text-[#ff3d77]" />}
                ...
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <button onClick={() => watchlist.toggleFavorite(movie.id)} className={`size-8 rounded-full glass flex items-center justify-center transition ${it.favorite ? "text-[#ff3d77]" : "hover:text-[#ff3d77]"}`}>
