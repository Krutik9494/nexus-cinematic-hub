...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Your cinematic journey at a glance." },
    ],
  }),
  component: Profile,
});
...
function Profile() {
  ...
  const { data: movies = [] } = useQuery({
    queryKey: ["batch", ids, !!apiKey],
    queryFn: () => tmdbBatch({ data: { ids, apiKey } }),
                ...
                {stats.recent.map(({ it, m }) => (
                  <div key={m.id} className="relative flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      {it.favorite && <Heart className="size-3.5 fill-current text-favorite" />}
